...
public class PlayerSelection extends State implements Statemethods {
    ...
    //3 >> 2
        characterAnimations = new CharacterAnimation[2];
        int i = 0;
    ...
    public void draw(Graphics g) {
        ...
        //center
        drawChar(g, playerIndex , menuX + menuWidth / 2, menuY + menuHeight / 2 , true);

        //Right
        drawChar(g, 0 , menuX, menuY + menuHeight / 2, false); 

        //Left
        drawChar(g,  1, menuX + menuWidth, menuY + menuHeight / 2, false); 

    }

    private void drawChar(Graphics g, int playerIndex, int x, int y, boolean center) { 
        if (playerIndex < 0)
            playerIndex = characterAnimations.length - 1;
        else if (playerIndex >= characterAnimations.length)
            playerIndex = 0;
        characterAnimations[playerIndex].draw(g, x, y, center);
    }
    ...
    public class CharacterAnimation {
        ...
        // if center condition
        public void draw(Graphics g, int drawX, int drawY, boolean center) {
        	if (center)
        		 ...
        		 g.drawImage(animations[pc.getRowIndex(IDLE)][aniIndex],
                         drawX - pc.spriteW * scale / 4,
                         drawY - pc.spriteH * scale / 4,
                         pc.spriteW * scale / 2,
                         pc.spriteH * scale / 2,
                         null);
